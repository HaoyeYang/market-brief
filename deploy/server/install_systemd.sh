...
Installed but not started. Next:
  1. Edit /etc/market-brief.env; add MOONSHOT_API_KEY and ZAI_API_KEY (plus NVIDIA_API_KEY when available).
  2. Keep /etc/market-brief.env mode 600; systemd injects it before dropping privileges.
  3. sudo -u $SERVICE_USER -H $PROJECT/.venv/bin/python $PROJECT/market_clock.py --date YYYY-MM-DD --mode preopen
  4. systemctl start market-brief.timer market-brief-backup.timer market-brief-viewer.service
EOF
